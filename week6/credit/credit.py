...
if __name__ == "__main__":  # main function
    ...
    if card_type == 'INVALID':
        print(card_type)
        sys.exit(1)  # invalid, doesn't check the first digit
    if (not validate_length_card(card_type, length_card)):
        print('INVALID')
        sys.exit(2)  # invalid, doesn't check length
    if (not check_luhn(card_number, length_card)):
        print('INVALID')
        sys.exit(3)  # invalid, doesn't check luhn
